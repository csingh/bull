...
function setDefaultOpts(opts) {
  // Changed: add throttleID as an option for now, can change it for priority later but then I'll need to modify the scripts
  ...
  // hardcode default to merchant 1 for now
  _opts.throttleID =
    typeof _opts.throttleID == 'undefined' ? 'Merchant1' : opts.throttleID;
...
Job.fromId = function(queue, jobId) {
  ...
  // console.log("From job.js/fromID", jobId)
  if (!jobId) {
...
/* Changed: add something to check throttle ID 
   Do I need to pass the redis in since connection is already established??? 
*/

Job.prototype.getThrottleLimit = function(redisTable) {
  const throttle = this.opts.throttleID;
  console.log('In get Throttle limit: ');
  redisTable
    .get(throttle)
    .then(result => {
      console.log(result);
